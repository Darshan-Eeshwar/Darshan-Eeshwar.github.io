...
The first CSV import clears every sample dataset to prevent mixing real and fictional data. Subsequent CSV imports replace only the selected dataset. JSON replaces the entire workspace atomically. Invalid imports preserve the existing workspace. All processing happens locally in memory; closing or reloading the page discards data. Export a JSON backup to retain it. CSV templates and the in-page data contract list every required column and enum. Limits: 10 MB per import, 20,000 rows per dataset.
